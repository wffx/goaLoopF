...
| 模型 | 用途 | 关键约束 |
|---|---|---|
| `FuzzRunRequest` | CLI 参数 | `source`（相对路径）、`function`（`[\\w:][\\w:$~.<>-]*`）、`max_generation_loops: 1–20`、`fuzz_seconds: 1–86400` |
...
| 模型 | 用途 | 关键约束 |
|---|---|---|
| `PreprocessResult` | 预处理输出 | `ready` 与 `terminal_status` 互斥：`ready=True` 时不能有 `terminal_status`；`ready=False` 时必须有 `terminal_status` |
