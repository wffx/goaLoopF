...
| `SourceContext` | 函数级上下文 + SHA-256 | `kind` 区分 `target_function`/`incoming_tree`/`outgoing_tree`/`dependency`/`build`；目标片段带 `start_line`/`end_line`；`path` 必须相对、无 `..`、无 `\\`；`truncated` 标记预算截断 |
