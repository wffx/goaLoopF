...
kRepo 审计、指标和客观派生信号，最多输出 3 条建议。模型不可用或输出持续无效时不会
...
本地代码只采集 generation 轮次、模型调用、token、工具调用和阶段耗时等客观数值，不按
终态或阈值生成、排序或预置任何建议。模型必须引用本次运行事实，弱证据时可以少于 3 条
或不输出建议。模型调用失败时，`suggestions` 保持为空并记录原因。
